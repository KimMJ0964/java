...
public class B_String {
	...
	public void method02() {
		...
		System.out.println("str1에 hello이라는 문자열이 포함? " + str1.contains("Hello"));
		System.out.println("str1에 Bye이라는 문자열이 포함? " + str1.contains("Bye"));
		
		// 4. 문자열.substring(int beginIndex, [int endIndex]) : String
		//		-> 문자열을 beginIndex위치부터 endIndex - 1위치까지 추출해서 반환
		System.out.println(str1.substring(6));		// endIndex는 생략 가능
		System.out.println(str1.substring(6, 9));
		
		// 5. 문자열.replace(char oldChar, char newChar) : String
		//		-> 문자열에서 oldChar 문자를 newChar 문자로 변환한 채 문자열 리턴
		String str4 = str1.replace('l', 'c');
		System.out.println("str1 : " + str1);
		System.out.println("str4 : " + str4);
		
		// 6. 문자열.toUpperCase() : String -> 문자열을 전부 대문자로 변경해 반환
		//	  문자열.toLowerCase() : String -> 문자열을 전부 소문자로 변경해 반환
	...
	public void method3() {
		...
		//		 String 클래스에서 제공하는 split 메서드를 사용
		...
		// System.out.println(stn.nextToken()); 더 이상 토큰이 남아있지 않을 때, 다음 토큰을 가져오려고 하면 예외발생
		...
		// hashMoreElements -> 가져올 토큰이 남아있는지 확인해주는 메소드 남아있다면 true 반환 없다면 false 반환
		stn = new StringTokenizer(str,",");
