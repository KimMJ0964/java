...
public class Book {
	/*
	 * 접근제한자 : 해당 구조에 접근할 수 있는 범위를 제한
	 * public > protected > default > private
	 * 
	 * 클래스에 사용 가능한 접근 제한자 2가지 : default, public
	 * 
	 * 필드와 메소드에 사용할 수 있는 접근 제한자 4가지
	 * public : 어디서든(같은 패키지, 다른 패키지) 접근 가능
	 * protected : 같은 패키지 + 다른 패키지일 경우, 상속 관계에서만 접근가능
	...
	// 생성자
	// 객체를 생성할 떄 데이터를 초기화해주는 작업이 필수적으로 필요하기에
	// 초기화를 위한 메서드를 만들어준다. -> 생성자라는 규칙이 생김
...
//	}
	// 생성자 : 클래스의 이름과 동일한 메소드로 반환형이 없다.
	//		  필드 데이터의 초기화를 위한 특수 목적의 메소드이다.
	// 		  매개변수가 하나도 없는 생성자를 우리는 기본생성자라 한다.
	//		  개발자가 직접 만들지 않는다면 컴파일러는 기본 생성자를 만들어준다.
	//		  개발자가 직접 생성자를 작성한다면 기본 생성자를 만들어주지 않는다.
	//		  메소드의 매개변수의 갯수 또는 타입에 따라서 메소드를 구분하는 메소드 오버로딩은 생성자에도 적용이 된다.
	public Book() {	System.out.println("기본 생성자 호출"); }
	...
	public String toString() {
		// this -> 인스턴스 객체(메모리를 사용중인) 자기 자신을 의미
		// 1. 자기 자신의 메모리를 가르킨다.
