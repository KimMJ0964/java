...
public class Run {
	 ...
	 * 	- 자식 타입 -> 부모 타입으로 형변환
	 * 	- 자동형변환
	 *  - ex) Car c1 = new Sonata();   자식.부모메소드();
	 * 
	 * 2. DownCasting
	 * 	- 부모 타입 -> 자식 타입으로 형변환
	 * 	- 강제형변환 해야함
	 *  - ex) ((Sonata)c1).moveSonata();	((자식타입)부모타입).자식메소드();
	 *  
	 *  - 다형성 -
	 *  부모 타입으로부터 파생된 여러가지 타입의 자식 객체들을 부모클래스
	 *  타입 하나로 다룰수 있는 기술
	 * 
	 * - 다형성의 장점 -
	 */  
	  // 다형성 사용 전 : 
		Sonata[] sonataArr = new Sonata[5];
	   	...
	   	avanteArr[0] = new Avante("검은색", "가솔린", 2022);
	   
	   // 다형성 사용 후 : 
		Car[] carArr = new Car[4];
	   	carArr[0] = new Sonata("흰색", "가스", 2023);
	    carArr[1] = new Avante("검은색", "가솔린", 2022);
	    carArr[2] = new Sonata("흰색", "가스", 2020);
	    carArr[3] = new Avante("검은색", "가솔린", 2019); 
