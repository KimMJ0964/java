...
public class ListRun {
	 ...
	 * *배열의 단점과 컬렉션의 장점
	 *  - 배열의 단점
	 *		1. 배열은 크기를 지정해야한다. 새로운 값을 더 추가하고자 한다면 새로운 크기의 배열을 만들고
	 *			기존의 값을 전부 복사하는 코드를 작성해야한다.
	 *		2. 배열 중간에 값을 추가하거나 삭제하는 경우, 값을 매번 이동시켜주는 복잡한 코드를 직접 작성해야한다.
	 *		3. 한 공간에 한 타입의 데이터들만 저장이 가능하다.
	 *
	 *  - 컬렉션의 장점
	 *  	1. 크기를 지정해 줄 필요 없음 만일 지정해주도라도 더 많은 데이터들을 담으면 알아서 사이즈를 늘려준다.
	 *  	2. 중간에 값을 추가한다거나 삭제하는 경우, 값을 당겨주는 작업(알고리즘)을 직접 기술한 필요가 없다.
	 *  		단지 메소드 호출만으로 알아서 내부적으로 진행이 된다.
	 *  	3. 한 공간에 여러 타입의 데이터들을 저장할 수 있다.(단, 객체만 가능)
	 *  		한 타입만 담도록 제한할 수 도 있다.(제네릭 사용 가능)
	 */
	
	public static void main(String[] args) {
		List list = new ArrayList(3);	// 크기를 지정할 수도 있고 안해도 상관없다.
		
		System.out.println(list); 	// 비어있는 상태
		// System.out.println(list.size());	// size = 생성된 크기가 아닌 사용 중인 크기
		
		// 1. add(E e) : 리스트 공간 끝에 전달된 데이터를 추가시켜주는 메서드
		
		list.add(new Music("에피소드", "이무진"));
		list.add(new Music("비의 랩소드", "임재현"));
		...
		System.out.println(list); // list의 특징 : 순서를 유지하면서 담김(0번 인덱스부터 차례대로)
		...
		// 4. set(int index, E e);		: 해당 인덱스의 값을 전달받은 e객체로 변경
		...
		// 7. subList(int index1, int index2);		: List에서 특정 부분을 추출해서 새로운 List로 변환
		List sub = list.subList(0, 2);		// 0부터 2번인덱스 전까지
		System.out.println(sub);
		
		// 8. addAll(Collection c);		: 컬렉션을 통째로 뒤에 추가시켜주는 메소드
		list.addAll(sub);
		System.out.println(list); 	// list 뒤에 sub가 추가됨
		
		// 9. isEmpty();	: boolean으로 컬렉션이 비어있는지 묻는 메소드
